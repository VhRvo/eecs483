...
fn interpret(e: &Exp, mut v: Vec<(String, i64)>) -> Result<i64, String> {
    match e {
        ...
        Exp::Add1(e) => match interpret(e, v) {
            Ok(n) => Ok(n + 1),
            Err(message) => Err(format!(
                "Error in evaluating expression argument to Add1: {}",
                message
            )),
        },

        // Error handling is done differently here for the sake of variety
        Exp::Sub1(e) => interpret(e, v).and_then(|n| Ok(n - 1)).or_else(|message| {
            Err(format!(
                "Error in evaluating expression argument to Sub1: {}",
                message
            ))
        }),
